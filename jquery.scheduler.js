    ...
    var loadRsvns = function(date, rsvns, settings) {
        ...
        for (i = 0; i < rsvns.length; i++) {
            if (rsvns[i].date == date) {
                ...
                var start1  = rsvns[i].start.split(":");
                var end1    = rsvns[i].end.split(":");

                var start = start1[1] == '30' ? parseInt(start1[0]) + 0.5 : parseInt(start1[0]);
                var end = end1[1] == '30' ? parseInt(end1[0]) + 0.5 : parseInt(end1[0]);
